#!/usr/bin/env python3
"""
Fast routing - SIMPLIFIED VERSION (no KML generation to avoid hanging)

Generates waypoints in a simple grid pattern across the polygon.
Exports only to GeoJSON format.

Usage:
  python scripts/fast_routing_simple.py [--polygon POLYGON] [--output OUTPUT] [--spacing-m SPACING]
"""
import sys
...
def export_to_geojson(waypoints_utm, utm_crs, output_path):
    """Export waypoints to GeoJSON LineString."""
    # Convert to WGS84
    transformer = Transformer.from_crs(utm_crs, "EPSG:4326", always_xy=True)
    coords_wgs84 = [transformer.transform(x, y) for x, y in waypoints_utm]
    ...
    route_geojson = {
        ...
        "features": [{
            ...
            "properties": {
                "name": "Route",
                "total_length_m": sum(
                    ((coords_wgs84[i+1][0] - coords_wgs84[i][0])**2 + 
                     (coords_wgs84[i+1][1] - coords_wgs84[i][1])**2)**0.5 * 111000
                    for i in range(len(coords_wgs84) - 1)
                ) if len(coords_wgs84) > 1 else 0
            },
            "geometry": {
                "type": "LineString",
...
def main():
    ...
    p = argparse.ArgumentParser(description="Fast routing (simplified): polygon -> grid waypoints -> GeoJSON")
    ...
    print("\n" + "="*70)
    print("FAST ROUTING - REGULAR GRID WAYPOINTS (SIMPLIFIED)")
    print("="*70)
    ...
    try:
        ...
        export_to_geojson(waypoints_utm, utm_crs, geojson_path)
        ...
        print(f"\n[VIEW] View the route:")
        print(f"   GeoJSON: {geojson_path}")
        print(f"   (Import into QGIS or any GIS tool)")

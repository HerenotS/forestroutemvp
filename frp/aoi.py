...
def load_aoi(geojson_path: Optional[str], bbox: Optional[str]) -> Polygon:
    ...
    if geojson_path:
        ...
        # prefer the newer GeoSeries.union_all() when available to avoid
        # DeprecationWarning for `unary_union` in recent geopandas/shapely
        try:
            geom = gdf.geometry.union_all()
        except Exception:
            # fallback to older API (will be used on older geopandas)

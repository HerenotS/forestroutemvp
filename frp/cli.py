...
def run_pipeline(
    ...
    if run_astar and mode != "coverage":
        # pass node area and sweep count via meta so astar can compute coarse spacing
        cm = cost_meta.copy()
        cm["node_area_ha"] = node_area_ha
        cm["num_sweeps"] = len(sweep_lines)
        cm["time_limit_s"] = time_limit_s
        cm["mode"] = mode
...
def main():
    ...
    plan_p.add_argument("--node-area-ha", type=float, default=2.0, help="Node area in hectares for coarse graph spacing")
    ...
    if args.cmd == "plan":
        ...
        run_pipeline(
            ...
            run_astar=True,
            mode="plan",
            node_area_ha=getattr(args, "node_area_ha", 2.0),
            time_limit_s=1.0,
